...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  //return {};
    vector<double> solved_points;
  solved_points.push_back(solution.x[delta_start]);
  solved_points.push_back(solution.x[a_start]);
  for (int i = 0; i < N; ++i) {
    solved_points.push_back(solution.x[x_start + i]);
    solved_points.push_back(solution.x[y_start + i]);
  }
